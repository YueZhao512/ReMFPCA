...
power_algo = function(data,sparse_tuning_result,sparse_tuning_type,S_2 = NULL,G_half_inverse = NULL,G_half = NULL,type = "real"){
  # data = t(data)
  b_old = svd(data)$v[,1]
  errors = 10^60
...
cv_score_sparse = function(data,G,S,K_fold,sparse_tuning_single,sparse_tuning_type,shuffled_row,group_size,S_back,S_back_inverse){
  ...
  for (k in 1:K_fold) {
    ...
    v_test = power_algo(t(data_train),sparse_tuning_single,sparse_tuning_type,type = "CV")
...
cv_selection_joint = function(data,mvmfd_obj,smooth_tuning,sparse_tuning,sparse_tuning_type,K_fold,G,G_half,G_half_inverse,S,S_2,S_inverse,S_smooth,S_back,S_back_inverse){
  CV_score = 10^60
  result = c()
  count = 0
  ...
  for (smooth_index in 1:dim(smooth_tuning)[1]) {
    ...
    for (sparse_tuning_single in sparse_tuning) {
      ...
      if (sparse_tuning_single == 0) {
        ...
        sparse_score = cv_score_sparse(data,G,S[[smooth_index]],K_fold,sparse_tuning_single,sparse_tuning_type,shuffled_row,group_size,S_back[[smooth_index]],S_back_inverse[[smooth_index]])
      }
      test_temp = power_algo(data%*%G_half,sparse_tuning_single,sparse_tuning_type,S_2[[smooth_index]],G_half_inverse,G_half)
      v_temp = test_temp[[2]]
...
cv_selection_marginal = function(data,mvmfd_obj,smooth_tuning,sparse_tuning,sparse_tuning_type,K_fold,G,G_half,G_half_inverse,S_2,S_smooth){
  ...
  for (sparse_tuning_single in sparse_tuning) {
    ...
    if (sparse_tuning_single == 0) {
      ...
      sparse_score = cv_score_sparse(data,G,G_half_inverse,K_fold,sparse_tuning_single,sparse_tuning_type,shuffled_row,group_size,diag(ncol(data)),diag(ncol(data)))
    }
  ...
  for (smooth_index in 1:dim(smooth_tuning)[1]) {
    ...
    test_temp = power_algo(data%*%G_half,sparse_tuning_selection,sparse_tuning_type,S_2[[smooth_index]],G_half_inverse,G_half)
    v_temp = test_temp[[2]]
    smooth_score = cv_score_smooth(data,mvmfd_obj,G,G_half,S_smooth[[smooth_index]],v_temp,smooth_tuning=smooth_tuning[smooth_index,])
...
s_power_algorithm <- function(mvmfd_obj, n, smooth_tuning, smooth_tuning_type,sparse_tuning, sparse_tuning_type,centerfns, alpha_orth,K_fold = 10,cv_type = "joint") {
  p <- mvmfd_obj$nvar
  ...
  if (alpha_orth == FALSE) {
    ...
    for (i in 1:n) {
      ...
      if (cv_type == "joint") {
        ...
        test_result = power_algo(B_temp%*%G_half,sparse_result,sparse_tuning_type,S_2[[smooth_result_index]],G_half_inverse,G_half)
      }
      else if (cv_type == "marginal") {
        cv_result = cv_selection_marginal(data = B_temp, mvmfd_obj = mvmfd_obj, smooth_tuning = smooth_tuning, sparse_tuning = sparse_tuning, sparse_tuning_type = sparse_tuning_type, K_fold = K_fold,G = G,G_half = G_half,G_half_inverse = G_half_inverse,S_2 = S_2,S_smooth = S_smooth)
