...
cv_local = function(data, G_half, K_fold, sparse_tuning_single, sparse_tuning_type, shuffled_row, group_size){
  ...
  for (k in 1:K_fold) {
    ...
    u_test = init_sequential(t(data_train), sparse_tuning_single, sparse_tuning_type, cv_flag = TRUE)
...
handle_smooth_tuning <- function(data, G_half, G, S_smooth, S_2_inverse, G_half_inverse, mvmfd_obj, sparse_tuning_selection = NULL, sparse_tuning_type = NULL, smooth_tuning, CV_score_smooth, power_type = "sequential", n = NULL, pb, count) {
  gcv_scores <- NULL
  ...
  if (is.null(smooth_tuning)) {
    ...
    for (smooth_index in 1:dim(smooth_tuning)[1]) {
      ...
      v_temp <- test_temp[[2]]
      smooth_score <- gcv_local(data, mvmfd_obj, G, G_half, S_smooth[[smooth_index]], v_temp, smooth_tuning = smooth_tuning[smooth_index, ])
...
gcv_joint <- function(data, G_half, G, S_smooth, S_2_inverse, G_half_inverse, mvmfd_obj, smooth_tuning, n) {
  CV_score_smooth <- Inf
  pb <- txtProgressBar(min = 0, max = dim(smooth_tuning)[1], style = 3, width = 50, char = "=")
  smooth_tuning_result <- handle_smooth_tuning(data, G_half, G, S_smooth, S_2_inverse, G_half_inverse, mvmfd_obj, smooth_tuning = smooth_tuning, CV_score_smooth = CV_score_smooth, power_type = "joint", n = n, pb = pb, count = 0)
  smooth_tuning_selection <- smooth_tuning_result$smooth_tuning_selection
  index_selection <- smooth_tuning_result$index_selection
  gcv_scores <- smooth_tuning_result$gcv_scores
...
handle_variance_update <- function(i, n, C, G, v_total, mvmfd_obj, all_equal_check, sparse_tuning, pc, lsv, v, u, G_half, test_result, temp_count, p) {
  for (j in 1:p) {
  ...
  if (i == 1 || all(all_equal_check) & (is.null(sparse_tuning) || all(unique(sparse_tuning) == 0))) {
    ...
    total_variance = sum(diag((coef_pc%*%t(v_total)) %*% G %*% t(coef_pc%*%t(v_total))))
...
sequential_power <- function(mvmfd_obj, n, smooth_tuning, smooth_tuning_type, sparse_tuning, sparse_tuning_type, centerfns, alpha_orth, K_fold, sparse_CV, smooth_GCV) {
  ...
  if (smooth_GCV == FALSE) {
    ...
    for (i in 1:n) {
      ...
      D <- ReMFPCA:::I_alpha(mvmfd_obj, smooth_tuning_temp) %*% smooth_penalty
      S_2 <- solve(G + D)
      ...
      test_result = init_sequential(C_temp%*%G_half, sparse_result, sparse_tuning_type, S_smooth[[1]], S_2_inverse[[1]], G_half_inverse, G_half)
      ...
      temp_count <- 0
      variance_result <- handle_variance_update(i, n, C, G, v_total, mvmfd_obj, all_equal_check, sparse_tuning, pc, lsv, v, u, G_half, test_result, temp_count, p)
  ...
  #########sequential inputs of smoothing parameters###########
    ...
    for (smooth_index in 1:dim(smooth_tuning_temp)[1]) {
      ...
      D <- ReMFPCA:::I_alpha(mvmfd_obj, smooth_tuning_temp[smooth_index, ]) %*% smooth_penalty
      S_2 <- solve(G + D)
...
joint_power <- function(mvmfd_obj, n, smooth_tuning, smooth_tuning_type, centerfns, alpha_orth) {
  ...
  test_result = init_joint(C%*%G_half, S_smooth[[smooth_result_index]], S_2_inverse[[smooth_result_index]], G_half_inverse, G_half, n = n)
  u = test_result[[2]]
  v = test_result[[1]]
